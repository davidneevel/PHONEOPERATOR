...
def lookForTwitHearts():
    ...
    the_cascade = cv2.CascadeClassifier('camera/cascades/twit_heart3.xml')
    ...
    hearts = the_cascade.detectMultiScale(roi)  # previously looked at gray instead of cropped roi
    ...
    if numHearts == 1:
        ...
        for (x,y,w,h) in hearts:
            ...
            if b <= 5:
                ...
                pc.tap()
                pc.type("look at this")
                pc.movexy(2800, 6350)  #tweet it
                pc.tap()
                sleep(2)
